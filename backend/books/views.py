...
from django.db.models import Count, OuterRef, Subquery, F, Case, When, Value, Func, ExpressionWrapper, FloatField
...
class ListCreateBookAPIView(ListCreateAPIView, BookImageCreator):
    ...
    def annotate_num_related_books(self, query_set):
        ...
        # subquery = RelatedBookGroup.objects.filter(
        #     related_books=OuterRef('pk')
        # )
        # test = [book for book in query_set if book.related_book_group_id == 2]
        # rb = RelatedBookGroup.objects.filter(related_books=test[0]).values_list()
